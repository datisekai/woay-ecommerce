...
const Cart = () => {
  ...
  useEffect(() => {
    const confirmCart = async () => {
      ...
      const preResult = result.map((item) => {
      ...
      const endResult = preResult.filter(
        (item) => item.quantityStore >= item.quantity
      );
      setCart(
        endResult.map((item) => ({
          variantId: item.id,
                      ...
                      {shippingInfo && (
                        <>
                          ...
                          <ModalChooseAddress
                            ...
                            data={userInfo}
                            elementClick={
                              <div className="btn mt-2 mr-1 btn-sm text-secondary">
                                Thay đổi
                              </div>
                            }
                          />
