...
const ModalUpdateColor = ({ data, handleHidden }) => {
  const queryClient = useQueryClient();

  const formikRef = useRef(null);

  const { mutate, isLoading } = useMutation(ColorApi.update, {
    onSuccess: (response, variables) => {
      const oldSizes = queryClient.getQueryData(["colors"]);
            ...
            >
              {({ handleSubmit }) => (
                <form onSubmit={handleSubmit}>
                  <h3 className="font-bold text-lg">Cập nhật Color</h3>
